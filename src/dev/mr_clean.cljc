...
(defn notify-watchers [watchers & path]
  #_(if path
...
(deftype RAtom [^:mutable value watchers cursors]
  ...
  (-reset! [this new-value]
    (when (not= value new-value)
      ...
      (let [filtered-cursors (filterv #(= [:person/id 2] (.-path  %)) @cursors)
            ]
        (doseq [c filtered-cursors      ;@cursors
                :let [cursor-watchers (.-watchers c)
                      cursor-path (.-path c)]]
          (println cursor-watchers)
